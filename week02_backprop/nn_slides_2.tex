...
\begin{frame}
\maketitle
\centering \textbf{\color{blue} Посиделка 2:}  Алгоритм обратного распространения ошибки
\end{frame}
...
\begin{center}
	...
	\only<3>{ 
		...
		\draw [->, dashed, ultra thick, red]  (second_2) to (first_1);
	}

	\only<4>{ 
		\draw [->, dashed, ultra thick, blue]  (fourth) to (third_2);
		\draw [->, dashed, ultra thick, amethyst]  (third_2) to (second_1);
		\draw [->, dashed, ultra thick, red]  (second_1) to (first_1);
	}

	\only<5>{ 
		\draw [->, dashed, ultra thick, blue]  (fourth) to (third_2);
		\draw [->, dashed, ultra thick, amethyst]  (third_2) to (second_2);
		\draw [->, dashed, ultra thick, red]  (second_2) to (first_1);
	}
...
\begin{center}
	\begin{tikzpicture}
		...
		\draw [->, dashed]   (output) -- (o2)node[pos=.49, above] {$\frac{\partial H_3}{\partial O_2}$} ;
		\draw [->, dashed]  (mse) -- (output) node[pos=.49, above] {$\frac{\partial MSE}{\partial \hat{y} }$} ;	
		
		\draw [->, dashed]  (9, -0.05) -- (9, -1.2)  node[pos=.49, right] {$\frac{\partial H_3}{\partial W_3} = O_2$} ;
